...
export const VkIcon = ({ dark }: { dark?: boolean }) => (
    <IB href='https://vk.com/smalleruditevkk'>
        <SvgIcon sx={{ fontSize: 26 }}>
            <svg fill={dark ? '#303030' : "#fff"} width="800px" height="800px" viewBox="0 0 32 32" version="1.1" xmlns="http://www.w3.org/2000/svg">
...
const MyDialog = ({
    ...
    open: boolean, setOpen: TSetBool, children: React.ReactNode
}) => (
    <Dialog
        open={open}
    ...
    >
        <Box
            className='flex flex-col items-center p-6 gap-5'
        >
            {children}
        </Box>
    </Dialog>
...
export const InstagramIcon = ({ dark }: { dark?: boolean }) => {
    ...
    return (
        <>
            ...
            <IB
                f={() => setOpen(true)}
            >
                <Instagram sx={{ ...style, color: dark ? '#303030' : '#fff' }} />
            </IB>
        </>
    )
}
...
export const PhoneCall = (
    ...
    return (
        <>
            <MyDialog open={open} setOpen={setOpen}>
                <Link href={'tel:' + mainInfo.phone} target='_blank'>
            ...
            {
                icon ?
                    <IB f={() => setOpen(true)}>< Phone sx={{ ...style, color: dark ? '#303030' : '#fff' }} /></IB >
                    :
                    <Typography
